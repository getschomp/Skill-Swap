...
# Begin:Two users that would create a match
    ...
    us2 = WantedSkill.create(skill_id: skill2.id, user_id: u1.id)
    us3 = HadSkill.create(skill_id: skill1.id, user_id: u2.id, expertise_level: "Novice")
    us4 = WantedSkill.create(skill_id: skill2.id, user_id: u2.id)
# End:Two users that would create a match


    location = Location.create(country: "USA", city:"Boston", state: "MA")
    location = Location.create(country: "USA", city:"Boston", state: "MA")
